...
def create_user(db: Session, user: schemas.User):
    ...
    db.add(db_user)     #create文発行
    db.commit()         #dbへコミット
    db.refresh(db_user) #dbの最新情報をインスタンスに反映
    return db_user

#会議室登録
def create_room(db: Session, room: schemas.Room):
    db_room = models.Room(room_name = room.room_name,capacity = room.capacity)
    db.add(db_room)     #create文発行
    db.commit()         #dbへコミット
    db.refresh(db_room) #dbの最新情報をインスタンスに反映
    return db_room
...
def create_booking(db: Session, booking: schemas.Booking):
    ...
    if len(db_booked) == 0:
        ...
        db.add(db_booking)      #create文発行
        db.commit()             #dbへコミット
        db.refresh(db_booking)  #dbの最新情報をインスタンスに反映
        return db_booking
    else:
